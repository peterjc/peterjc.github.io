...
`-d` option will store all files in the same directory. Otherwise, they
are sorted into PDB-style subdirectories according to their PDB ID's.
...
ever - I don't need it at all, and it's a lot of work, plus no-one has
ever asked for it). People who want to add this can contact me.
...
-   The **insertion code**, e.g. `'A'`. The insertion code is sometimes
    ...
    residue) could e.g. have sequence identifiers and insertion codes as
    follows: Thr 80 A, Ser 80 B, Asn 81. In this way the residue
...
\\hline T& Turn\\tabularnewline \\hline S& Bend \\tabularnewline
\\hline -& Other\\tabularnewline \\hline \\end{tabular
...
`rotaxis function, the <code>Vector module also has methods
to rotate (<code>rotmat`) or reflect (`refmat`) one vector on top of
another.
